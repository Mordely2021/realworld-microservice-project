...
pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker build -t mordely2021/frontendservice:latest ."
                    }
                }
            }
        }
        ...
        stage('Snyk SCA Test | Dependencies') {
            steps {
                sh "${SNYK_HOME}/snyk-linux test --docker mordely2021/frontendservice:latest || true" 
            }
        }
        ...
        stage('Push Microservice Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker push mordely2021/frontendservice:latest "
                    }
                }
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {        // // Perform DAST Test on Application

            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no mordely@192.168.12.196 "docker run -t zaproxy/zap-weekly zap-baseline.py -t http://3.133.89.56:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#max-123-multi-microservieces-alerts', //update and provide your channel name
